...
mutable struct KSIntegrator
    ...
    Nn::Vector{ComplexF64}
    Nn1::Vector{ComplexF64}
    u::Vector{ComplexF64}
end
function KSIntegrator(u0, Lx, dt)
    ...
    # compute in-place planned FFTs
    FFT! = plan_fft!((1 + 0im) * u0; flags=FFTW.ESTIMATE)
    IFFT! = plan_ifft!((1 + 0im) * u0; flags=FFTW.ESTIMATE)

    # prepare time evolution in Fourier domain
    Nn::Vector{ComplexF64} = G .* fft(u0 .^ 2)
    Nn1::Vector{ComplexF64} = copy(Nn)
    u::Vector{ComplexF64} = FFT! * u0

    KSIntegrator(u0, Lx, dt, Nx, kx, alpha, L, G, dt2, dt32, A, B, FFT!, IFFT!, Nn, Nn1, u)
end
